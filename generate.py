...
<div class="main" style="display:none;">
  ...
  <div id="page-ranking" class="page">
    ...
    <div class="table-wrap"><table><thead><tr><th width="36">#</th><th>商品</th><th class="num">該当期間</th><th class="num">環比</th><th class="num">前年比</th></tr></thead><tbody id="rk-body"></tbody></table></div>
  </div>
  <!-- Inventory -->
  <div id="page-inventory" class="page">
    ...
    <div class="table-wrap"><table class="inv-table"><thead><tr>
      <th class="desktop-only">画像</th><th class="desktop-only">商品名</th><th class="desktop-only">カラー</th><th class="desktop-only" style="text-align:center">サイズ</th><th class="mobile-product">商品</th>
      <th class="stock">ハラカド</th><th class="stock">新宿</th><th class="stock">大阪</th><th class="stock">二子玉川</th><th class="stock">合計</th><th class="stock">EC在庫</th>
...
function renderSA() {{
  ...
  document.getElementById('sa-cards').innerHTML = metrics.map(m => '<div class="sa-card"><div class="sa-label">'+m.l+'</div><div class="sa-value">'+m.f(m.a)+'</div>'+(comparing&&sumB?pct(m.a,m.b)+'<div class="sa-sub sa-compare-val">比較: '+m.f(m.b)+'</div>':'')+'</div>').join('');
  // Chart
  const ctx=document.getElementById('sa-chart').getContext('2d');
  if(saChart)saChart.destroy();
  const ds=[{{label:f1+'～'+t1,data:a.map(d=>d.sales),borderColor:'#0984e3',backgroundColor:'rgba(9,132,227,.08)',fill:true,tension:.3,pointRadius:a.length>31?0:3,borderWidth:2}}];
...
function rkAggregate(store, from, to) {{
  ...
  for (const [k, q] of Object.entries(RK_RAW)) {{
    const [s,d,spu] = k.split('|');
    if (s===store && d>=from && d<=to) map[spu] = (map[spu]||0) + q;
  }}
  return map;
}}
...
function renderRanking() {{
  ...
  document.getElementById('rk-summary').innerHTML='<div class="rk-total"><span class="rk-total-num">'+tc.toLocaleString()+'</span><span class="rk-total-label">販売数</span></div><div class="rk-comp"><span class="rk-comp-label">環比</span>'+pctHtml(tc,tp)+'</div><div class="rk-comp"><span class="rk-comp-label">前年比</span>'+pctHtml(tc,ty)+'</div>';
  const medals={{0:'🥇',1:'🥈',2:'🥉'}};
  document.getElementById('rk-body').innerHTML=ranked.map(([spu,qty],i) => {{
    ...
    return '<tr><td class="rank">'+(medals[i]||(i+1))+'</td><td class="product-cell">'+img+'<div class="product-info"><div class="product-name">'+(info.name||spu)+'</div><div class="product-color">'+(info.color||'')+'</div></div></td><td class="num highlight">'+qty+'</td><td class="num">'+pctHtml(qty,prev[spu]||0)+'</td><td class="num">'+pctHtml(qty,yoy[spu]||0)+'</td></tr>';
  }}).join('');
}}
...
async function loadInventory() {{
  ...
  for (const r of parseCSV(invCSV)) {{
    const sku=r[0], info=skuInfo[sku]||{{}};
    INV_DATA.push({{ sku, img:info.img||'', upc:info.upc||'', cat:info.cat||'', name:info.name||sku, color:info.color||'', size:info.size||'',
      hara:parseInt(r[1])||0, shinjuku:parseInt(r[2])||0, osaka:parseInt(r[3])||0, futako:parseInt(r[4])||0, total:parseInt(r[6])||0, ec:ecInv[sku]||0 }});
  }}
...
function renderInventory() {{
  ...
  document.getElementById('inv-body').innerHTML=filtered.map(d => {{
    ...
    return '<tr><td class="desktop-only img-col">'+img+'</td><td class="name-col desktop-only">'+d.name+'</td><td class="color-col desktop-only">'+d.color+'</td><td class="size-col desktop-only">'+d.size+'</td><td class="mobile-product"><div class="m-product">'+img+'<div class="m-info"><div class="m-name">'+d.name+'</div><div class="m-detail">'+detail+'</div></div></div></td>'+sc(d.hara)+sc(d.shinjuku)+sc(d.osaka)+sc(d.futako)+sc(d.total)+sc(d.ec)+'</tr>';
  }}).join('');
}}
