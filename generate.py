...
<script>
/* ========================================================================
   VIVAIA Dashboard — JS 目次（grep で "SECTION:" を検索すれば各節に飛べる）
   ------------------------------------------------------------------------
   SECTION: auth_and_users     — USERS, sha256, unlockApp
   SECTION: prod_info_cache    — card 90 KV + localStorage 24h キャッシュ
   SECTION: helpers            — mbQuery/mbSQL/parseCSV/jstToday/quickRange/pctHtml
   SECTION: nav_and_mobile     — ページ切替、モバイルメニュー
   SECTION: page_analysis      — 売上分析（loadSalesAnalysis 以下）
   SECTION: page_ranking       — 商品ランキング（loadRanking 以下）
   SECTION: page_product       — 商品別売上（loadProductPage 以下）
   SECTION: page_inventory     — 在庫確認（loadInventory 以下）
   SECTION: page_receiving     — 入荷予定（loadReceiving 以下、5ページ目）
   ========================================================================
   注: 行番号はすぐにズレるので、上の SECTION: アンカーを grep してください。
   ページ追加時はアンカーを増やし、この目次も更新すること。
   ======================================================================== */
const W = '{WORKER}';

// ── Login ──
/* SECTION: auth_and_users — パスワード hash → role + 可視ページ */
const USERS = {{
...
// ── Helpers ──
/* SECTION: helpers — mbQuery/mbSQL/parseCSV/jstToday/addDays/quickRange/pctHtml */
...
/* SECTION: nav_and_mobile — ページ切替、モバイルハンバーガーメニュー */
...
/* SECTION: page_analysis — 売上分析 (loadSalesAnalysis → initSAControls → renderSA) */
...
/* SECTION: page_ranking — 商品ランキング (RK_RAW データ、loadRanking/renderRanking) */
...
/* SECTION: page_product — 商品別売上 (RK_RAW 共有、loadProductPage/renderProductPage) */
...
/* SECTION: page_inventory — 在庫確認 (card 90+120+137、loadInventory/renderInventory) */
...
/* SECTION: page_receiving — 入荷予定 (Smaregi API, loadReceiving/showRCDetail/rcBuildGroups) */
